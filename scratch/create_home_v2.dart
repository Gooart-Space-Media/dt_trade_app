...
void main() {
  ...
  int lotSize = lines.indexWhere((l) => l.contains('class LotSizeCalcPage'));
  
  // The closing brace of OverlapCheckerPageState is right before the LotSizeCalcPage comments
  int extractEnd = -1;
  for (int i = lotSize - 1; i > bannerStart; i--) {
    if (lines[i].trim() == '}') {
      extractEnd = i;
      break;
    }
  ...
  if (bannerStart == -1 || extractEnd == -1) {
    print('Failed to find the methods to move out of OverlapCheckerPage!');
    return;
  }

  List<String> methodsToMove = lines.sublist(bannerStart, extractEnd);
  ...
  lines.removeRange(bannerStart, extractEnd);
