...
with open('scratch/clean_class.dart', 'r', encoding='utf-8') as f:
    code = f.read()

# 1. Add checklist state
code = code.replace(
    "String dir3 = '多';",
...
# 4. Trades Row
old_trades = """        _buildDropdownRow('交易 1 (首选主线)', s1, dir1, allPairs, [], (p) => setState(() { s1 = p; s2 = null; s3 = null; }), (d) => setState(() => dir1 = d)),
...
# 5. Checklist UI & Close Container
old_radar_end = """            ],
          ),
        ),
      ],
...
with open('scratch/dirty_class.dart', 'w', encoding='utf-8') as f:
    f.write(code)

with open('lib/main.dart', 'r', encoding='utf-8') as f:
    main_code = f.read()

start_idx = main_code.find('class _OverlapCheckerPageState extends State<OverlapCheckerPage>')
end_idx = main_code.find('class _AvoidCard extends StatelessWidget') - 1

new_main = main_code[:start_idx] + code + main_code[end_idx:]
