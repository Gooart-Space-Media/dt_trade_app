const fs = require('fs');
let code = fs.readFileSync('scratch/clean_class.dart', 'utf-8');
...
let main_code = fs.readFileSync('lib/main.dart', 'utf-8');
const start_idx = main_code.indexOf('class _OverlapCheckerPageState extends State<OverlapCheckerPage>');
const end_idx = main_code.indexOf('class _AvoidCard extends StatelessWidget') - 1;

main_code = main_code.slice(0, start_idx) + code + main_code.slice(end_idx);
fs.writeFileSync('lib/main.dart', main_code);
console.log('Fixed main.dart successfully using node!');
