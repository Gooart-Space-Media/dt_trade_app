...
void main() {
  ...
  for (int i = 0; i < lines.length; i++) {
    if (lines[i].contains('Row(') && lines[i].trim() == 'Row(') {
      if (lines[i+1].contains('crossAxisAlignment: CrossAxisAlignment.start,')) {
         if (lines[i+2].contains('children: [')) {
            if (lines[i+3].contains('Expanded(')) {
               startIndex = i;
               break;
            }
         }
  ...
  if (startIndex != -1) {
    for (int i = startIndex; i < startIndex + 70; i++) {
      if (lines[i].contains('],')) {
        if (lines[i+1].contains('),') && lines[i+2].trim() == '') {
           if (lines[i+3].contains('动态变速箱风控档位')) {
              endIndex = i + 1;
              break;
           }
        }
