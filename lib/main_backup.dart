    ...
    return ListView(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const Text('🛡️ 多单并行防呆与自审', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
            Row(
              children: [
                TextButton.icon(
...
class _LotSizeCalcPageState extends State<LotSizeCalcPage> {
  ...
  Map<String, dynamic> _getFightIqDiagnosis(double pips) {
    if (isGoldMode) {
      return {
        'status': '🥇 黄金形态模式',
        'desc': '黄金 10 Pips = \$1.00 美元波动。建议止损设在结构外 20~30 Pips。',
        'color': const Color(0xFFD97706),
      };
    }
    if (pips < 50) {
  ...
  Widget build(BuildContext context) {
    ...
    double rawLots = (slPips > 0) ? riskAmt / (slPips * 10) : 0;
            ...
            children: [
              Expanded(
                child: GestureDetector(
                  onTap: () {
                    HapticFeedback.selectionClick();
                    setState(() => isGoldMode = false);
                    _saveParam('calc_is_gold', false);
                  },
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 8),
                    decoration: BoxDecoration(
                      color: !isGoldMode ? const Color(0xFF2563EB) : Colors.transparent,
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Center(
                      child: Text('💱 标准外汇货币对', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: !isGoldMode ? Colors.white : Colors.grey)),
                    ),
                  ),
                ),
              ),
              Expanded(
                child: GestureDetector(
                  onTap: () {
                    HapticFeedback.selectionClick();
                    setState(() => isGoldMode = true);
                    _saveParam('calc_is_gold', true);
                  },
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 8),
                    decoration: BoxDecoration(
                      color: isGoldMode ? const Color(0xFFD97706) : Colors.transparent,
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Center(
                      child: Text('🥇 现货黄金 (XAUUSD)', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isGoldMode ? Colors.white : Colors.grey)),
                    ),
                  ),
                ),
              ),
          ...
          children: [2, 3, 4].map((r) {
            ...
            return Expanded(
              child: Padding(
                ...
                child: ChoiceChip(
                  label: Center(child: Text('$r% ${r == 2 ? "(巡航)" : r == 3 ? "(激进)" : "(极限)"}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 11))),
                  selected: selected,
                  selectedColor: isGoldMode ? const Color(0xFFFEF3C7) : const Color(0xFFEFF6FF),
                  onSelected: (_) {
                    HapticFeedback.lightImpact();
                    setState(() => riskPct = r);
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                Text('≈ RM ${riskAmtRM.toStringAsFixed(2)}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF059669))),
...
class _TpCalculatorPageState extends State<TpCalculatorPage> {
  ...
  bool isLong = true;
  bool isFibMode = true; // 默认采用黄金口袋模式
  bool isGold = false;
  ...
  void _addCurrentPlan({
    ...
    HapticFeedback.mediumImpact();
    final entry = isFibMode ? fib50.toStringAsFixed(decimals) : breakoutEntry.toStringAsFixed(decimals);
    final sl = isFibMode ? fibSl.toStringAsFixed(decimals) : breakoutSl.toStringAsFixed(decimals);
    final tp1 = isFibMode
  ...
  Widget build(BuildContext context) {
    ...
    double bufferEntry = 10 / pipMultiplier;
    double bufferSl = 9 / pipMultiplier;
    ...
    return ListView(
      ...
      children: [
            ...
            children: [
              Expanded(
                child: GestureDetector(
                  onTap: () {
                    HapticFeedback.selectionClick();
                    setState(() => isFibMode = true);
                  },
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 8),
                    decoration: BoxDecoration(
                      color: isFibMode ? const Color(0xFFD97706) : Colors.transparent,
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Center(
                      child: Text('🎯 黄金口袋 (Fib 50~61.8%)', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: isFibMode ? Colors.white : Colors.grey)),
                    ),
                  ),
                ),
              ),
              Expanded(
                child: GestureDetector(
                  onTap: () {
                    HapticFeedback.selectionClick();
                    setState(() => isFibMode = false);
                  },
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 8),
                    decoration: BoxDecoration(
                      color: !isFibMode ? const Color(0xFF2563EB) : Colors.transparent,
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Center(
                      child: Text('🚀 常规突破 (+10pips缓冲)', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: !isFibMode ? Colors.white : Colors.grey)),
                    ),
                  ),
                ),
              ),
        ...
        TextFormField(
          ...
          onChanged: (v) => setState(() => highPrice = double.tryParse(v)),
        ),
        const SizedBox(height: 12),
        TextFormField(
          controller: _lowCtrl,
          ...
          onChanged: (v) => setState(() => lowPrice = double.tryParse(v)),
        ),
        const SizedBox(height: 16),
        ...
        if (hasData) ...[
          ...
          Card(
            ...
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(16),
              side: BorderSide(color: isFibMode ? const Color(0xFFFDE68A) : const Color(0xFFBFDBFE)),
            ),
            color: isFibMode ? const Color(0xFFFFFDF5) : const Color(0xFFF8FAFC),
            child: Padding(
              padding: const EdgeInsets.all(14),
              child: Column(
                children: [
                  Row(
                    ...
                    children: [
                      Text(
                        '💰 双轨联动推荐手数 (本金 \$${userBalance.toStringAsFixed(0)} · 风控 $userRisk%)',
                        style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isFibMode ? const Color(0xFFB45309) : const Color(0xFF1E40AF)),
                      ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildCheckItem('📏 2. 动能健康：极值在 50~100 Pips 黄金区间，非噪音非衰竭', _checkFightIq, (v) => setState(() => _checkFightIq = v ?? false)),
                  _buildCheckItem('🛡️ 3. 双缓冲保护：入场+10pips假破缓冲，止损+9pips结构缓冲', _checkBuffer, (v) => setState(() => _checkBuffer = v ?? false)),
            ...
            children: [
              Expanded(
                ...
                child: FilledButton.icon(
                  style: FilledButton.styleFrom(
                    backgroundColor: isFibMode ? const Color(0xFFD97706) : const Color(0xFF2563EB),
              ...
              Expanded(
                ...
                child: OutlinedButton.icon(
                  style: OutlinedButton.styleFrom(
                    padding: const EdgeInsets.symmetric(vertical: 13),
                    side: BorderSide(color: isFibMode ? const Color(0xFFD97706) : const Color(0xFF2563EB), width: 1.5),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                  ),
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 10),
                  const Text('等待输入蜡烛极值', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                  const SizedBox(height: 6),
                  Text(
                  ...
                  children: [
                    ...
                    if (_savedPlans.isNotEmpty)
                      TextButton(
                        ...
                        child: const Text('清空计划', style: TextStyle(fontSize: 11, color: Colors.grey)),
                      ),
                  ],
                ),
                const SizedBox(height: 10),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('双轨分仓: ${item['lot']} + ${item['lot']} 手', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600)),
  ...
  Widget _buildPairChipV2(WatchlistPair p) {
    ...
    return ChoiceChip(
      selected: isSelected,
      selectedColor: p.isCore
          ? const Color(0xFFFEF3C7)
          : (p.isGold ? const Color(0xFFFDE68A) : const Color(0xFFDBEAFE)),
      avatar: p.isCore
      ...
      onSelected: (_) => _selectWatchlistPair(p),
    );
  }
  ...
  void _copyFullTradePlan({
    ...
    HapticFeedback.mediumImpact();
    final pairInfo = getWatchlistPair(selectedPair);
    final orderType = isFibMode
        ? (isLong ? 'Buy Limit (回踩挂多)' : 'Sell Limit (反弹挂空)')
        : (isLong ? 'Buy Stop (突破追多)' : 'Sell Stop (跌破追空)');
    final entryPrice = isFibMode ? fib50.toStringAsFixed(decimals) : breakoutEntry.toStringAsFixed(decimals);
    final slPrice = isFibMode ? fibSl.toStringAsFixed(decimals) : breakoutSl.toStringAsFixed(decimals);
    final tp1Price = isFibMode
        ? (isLong ? fib50 + fib50Risk : fib50 - fib50Risk).toStringAsFixed(decimals)
        : (isLong ? breakoutEntry + breakoutRisk : breakoutEntry - breakoutRisk).toStringAsFixed(decimals);
    final tp2Price = isFibMode
        ? (isLong ? fib50 + fib50Risk * 2 : fib50 - fib50Risk * 2).toStringAsFixed(decimals)
        : (isLong ? breakoutEntry + breakoutRisk * 2 : breakoutEntry - breakoutRisk * 2).toStringAsFixed(decimals);
    final tp3Price = isFibMode
...
● 挂单模式：${isFibMode ? "🎯 黄金口袋 (Fib 50% 回踩狙击)" : "🚀 极值突破挂单 (+10pips假破缓冲)"}
● 挂单类型：$orderType
● 挂单价格：$entryPrice
...
----------------------------------------
$checklistNote⚠️ 纪律红线：Set & Forget！挂单后绝不手动追单，到 1:1 必须严格推保本！
''';
