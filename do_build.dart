...
void main() {
  String code = File('scratch/old_build.dart').readAsStringSync();
  
  // We need to parse this and extract the parts we want.
  // We know the exact string matches for what we want.
  ...
  String inputs1 = '''
            Expanded(
              child: TextFormField(
                ...
                decoration: InputDecoration(isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8), labelText: '本金 (USD)', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))),
                onChanged: (v) {
            ...
            Expanded(
              child: TextFormField(
                controller: _winCtrl,
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                decoration: InputDecoration(isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8), labelText: '胜率 (\\${riskPct.toStringAsFixed(0)}%)', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))),
                onChanged: (v) {
  ...
  String inputs2 = '''
            Expanded(
              child: TextFormField(
                ...
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                decoration: InputDecoration(isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8), labelText: '单笔风险 (\\${riskPct.toStringAsFixed(0)}%)', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))),
                onChanged: (v) {
            ...
            Expanded(
              child: DropdownButtonFormField<double>(
                value: rr,
                decoration: InputDecoration(isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8), labelText: '盈亏比 (RR)', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))),
                items: const [
        ...
        final leftContent = Column(
          ...
          children: [
            ...
            if (constraints.maxWidth > 800)
              ...
              Column(
                children: [
                  Row(children: [ $inputs1 ]),
                  const SizedBox(height: 8),
                  Row(children: [ $inputs2 ]),
                ],
              ),
            const SizedBox(height: 8),
            FilledButton.icon(
            ...
            child: Column(
              ...
              children: [
                ...
                if (simResult != null) ...[
                  ...
                  SizedBox(height: 500, child: rightContent),
                ]
              ],
            ),
          );
...
''';
  ...
  String mainCode = File('lib/main.dart').readAsStringSync();
  ...
  for (int i=3700; i<lines.length; i++) {
    ...
    if (start != -1 && lines[i].contains('Widget _buildLegend(Color color, String text) {')) {
      end = i;
      break;
    }
  }

  String finalCode = lines.sublist(0, start).join('\n') + '\n' + newBuild + '\n' + lines.sublist(end).join('\n');
